...
public class SignInManagerExtension<TUser> : SignInManager<AppUser> where TUser : class
{
    ...
    public SignInManagerExtension(UserManager<AppUser> userManager,
    ...
    public virtual async Task<SignInResult> PasswordSignInAsync(AppUser user, string password, bool isPersistent, bool lockoutOnFailure, List<Claim> customClaims)
    {
    ...
    public async Task<SignInResult> SignInUserAsync(AppUser user, string password, bool isPersistent, bool lockoutOnFailure, List<Claim> customClaims)
    {
        ArgumentNullException.ThrowIfNull(user);
    ...
    public new virtual async Task<SignInResult> ExternalLoginSignInAsync(string loginProvider, string providerKey, bool isPersistent, bool bypassTwoFactor)
    {
        ...
        return error ?? await SignInOrTwoFactorAsync(user, isPersistent, [], loginProvider, bypassTwoFactor);
    }
    ...
    public new virtual async Task RefreshSignInAsync(AppUser user)
    {
    ...
    {
        SignInManagerExtension<TUser>.TwoFactorAuthenticationInfo? twoFactorInfo = await RetrieveTwoFactorInfoAsync();
        if (twoFactorInfo == null)
        {
            return SignInResult.Failed;
        }

        AppUser user = twoFactorInfo.User;
        SignInResult? error = await PreSignInCheck(user);
        if (error != null)
        {
            return error;
    ...
    protected virtual async Task<SignInResult> SignInOrTwoFactorAsync(AppUser user, bool isPersistent, List<Claim> customClaims, string? loginProvider = null, bool bypassTwoFactor = false)
    {
        customClaims ??= [];
    ...
    /// <returns>The task object representing the asynchronous operation.</returns>
    public virtual Task SignInAsync(AppUser user, bool isPersistent, List<Claim> customClaims, string? authenticationMethod = null)
    {
        return SignInAsync(user, new AuthenticationProperties { IsPersistent = isPersistent }, customClaims, authenticationMethod);
    }
    ...
    /// <param name="authenticationMethod">Name of the method used to authenticate the user.</param>
    /// <returns>The task object representing the asynchronous operation.</returns>
    public virtual Task SignInAsync(AppUser user, AuthenticationProperties authenticationProperties, List<Claim> customClaims, string? authenticationMethod = null)
    {
        customClaims ??= [];
    ...
    private async Task<IdentityResult> ResetLockoutWithResult(AppUser user)
    {
    ...
    private async Task<TwoFactorAuthenticationInfo?> RetrieveTwoFactorInfoAsync()
    {
        ...
        return user == null
            ? (SignInManagerExtension<TUser>.TwoFactorAuthenticationInfo?)null
    ...
    private async Task<SignInResult> DoTwoFactorSignInAsync(AppUser user, TwoFactorAuthenticationInfo twoFactorInfo, bool isPersistent, bool rememberClient, List<Claim> claims)
    {
    ...
    private sealed class IdentityResultException : Exception
    {
        ...
        public override string Message
        {
            get
            {
                ...
                {
                    _ = sb.AppendLine();
                    _ = sb.Append(error.Code);
                    _ = sb.Append(": ");
                    _ = sb.Append(error.Description);
                }
