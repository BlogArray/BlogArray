...
public class GetMediaQuery(int pageNumber, int pageSize, string? searchTerm) : IRequest<PagedResult<MediaInfo>>
{
...
public class GetMediaQueryHandler(IMediaRepository mediaRepository) : IRequestHandler<GetMediaQuery, PagedResult<MediaInfo>>
{
    ...
    {
        return await mediaRepository.GetPaginatedAsync(request.PageNumber, request.PageSize, request.SearchTerm);
    }
}
